...
DECLARE_CYCLE_STAT(TEXT("Create ProcMesh Proxy"), STAT_ProcMesh_CreateSceneProxy, STATGROUP_JetProceduralMesh);
DECLARE_CYCLE_STAT(TEXT("Create Mesh Section"), STAT_ProcMesh_CreateMeshSection, STATGROUP_JetProceduralMesh);
DECLARE_CYCLE_STAT(TEXT("UpdateSection GT"), STAT_ProcMesh_UpdateSectionGT, STATGROUP_JetProceduralMesh);
DECLARE_CYCLE_STAT(TEXT("UpdateSection RT"), STAT_ProcMesh_UpdateSectionRT, STATGROUP_JetProceduralMesh);
DECLARE_CYCLE_STAT(TEXT("Get ProcMesh Elements"), STAT_ProcMesh_GetMeshElements, STATGROUP_JetProceduralMesh);
DECLARE_CYCLE_STAT(TEXT("Update Collision"), STAT_ProcMesh_UpdateCollision, STATGROUP_JetProceduralMesh);
...
public:
	...
	TArray<FJetProcMeshVertex> NewVertexBuffer;
};

static void ConvertProcMeshToDynMeshVertex(FDynamicMeshVertex& Vert, const FJetProcMeshVertex& ProcVert)
{
...
public:
	...
	FProceduralMeshSceneProxy(UJetProceduralMeshComponent* Component)
		...
		, MaterialRelevance(Component->GetMaterialRelevance(GetScene().GetFeatureLevel()))
	{
		...
		{
			FJetProcMeshSection& SrcSection = Component->ProcMeshSections[SectionIdx];
			if (SrcSection.ProcIndexBuffer.Num() > 0 && SrcSection.ProcVertexBuffer.Num() > 0)
			{
				...
				{
					const FJetProcMeshVertex& ProcVert = SrcSection.ProcVertexBuffer[VertIdx];
	...
	void UpdateSection_RenderThread(FProcMeshSectionUpdateData* SectionData)
	{
		...
		if (SectionData != nullptr)
		{
				...
				Sections[SectionData->TargetSection] != nullptr)
			{
				...
				{
					const FJetProcMeshVertex& ProcVert = SectionData->NewVertexBuffer[i];
...
UJetProceduralMeshComponent::UJetProceduralMeshComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	bUseComplexAsSimpleCollision = true;
}

void UJetProceduralMeshComponent::PostLoad()
{
	Super::PostLoad();
...
void UJetProceduralMeshComponent::CreateMeshSection(int32 SectionIndex, const TArray<FVector>& Vertices, const TArray<int32>& Triangles, const TArray<FVector>& Normals, const TArray<FVector2D>& UV0, const TArray<FVector2D>& UV1, const TArray<FVector2D>& UV2, const TArray<FVector2D>& UV3, const TArray<FColor>& VertexColors, const TArray<FJetProcMeshTangent>& Tangents, bool bCreateCollision)
{
	...
	FJetProcMeshSection& NewSection = ProcMeshSections[SectionIndex];
	NewSection.Reset();
	...
	{
		FJetProcMeshVertex& Vertex = NewSection.ProcVertexBuffer[VertIdx];
		...
		Vertex.Tangent = (Tangents.Num() == NumVerts) ? Tangents[VertIdx] : FJetProcMeshTangent();
...
{
	FOnAsyncUpdateMeshSectionCompleteDelegate Out;

	Out.AddDynamic(this, &UJetProceduralMeshComponent::OnAsyncUpdateMeshSectionComplete);

	if (SectionIndex < ProcMeshSections.Num())
	{
		TArray<FJetProcMeshSection>* ProcMeshSectionsPtr = &ProcMeshSections;

		FBodyInstance* BodyInstancePtr = &BodyInstance;

		TArray<FVector>* AsyncCollisionPositionsPtr = &AsyncCollisionPositions;

		AsyncTask(ENamedThreads::AnyHiPriThreadNormalTask, [Out, SectionIndex, Vertices, Normals, UV0, UV1, UV2, UV3, VertexColors, Tangents, ProcMeshSectionsPtr, BodyInstancePtr, AsyncCollisionPositionsPtr]()
		{
			TArray<FJetProcMeshSection>& LocalProcMeshSections = *ProcMeshSectionsPtr;

			FBodyInstance& LocalBodyInstance = *BodyInstancePtr;

			FJetProcMeshSection& Section = LocalProcMeshSections[SectionIndex];

			TArray<FVector>& AsyncCollisionPositions = *AsyncCollisionPositionsPtr;
			...
			if (bSameVertexCount)
			{
				...
				if (Section.bEnableCollision)
				{
					...
					for (const FJetProcMeshSection& CollisionSection : LocalProcMeshSections)
					{
						...
						if (CollisionSection.bEnableCollision)
						{
							...
							{
								AsyncCollisionPositions.Add(CollisionSection.ProcVertexBuffer[VertIdx].Position);
							}
						}
					}
				}
			...
			AsyncTask(ENamedThreads::GameThread, [Out, SectionIndex]()
			{
				// We execute the delegate
				Out.Broadcast(SectionIndex);
			});
		});
...
void UJetProceduralMeshComponent::OnAsyncUpdateMeshSectionComplete(int32 InMeshSectionIndex)
{
	int32 sixnine = 69;


	if (InMeshSectionIndex >= ProcMeshSections.Num())
	{
		return;
	}

	// Pass new positions to trimesh
	BodyInstance.UpdateTriMeshVertices(AsyncCollisionPositions);

	AsyncCollisionPositions.Empty();

	FJetProcMeshSection& Section = ProcMeshSections[InMeshSectionIndex];
	...
	if (SceneProxy && !IsRenderStateDirty())
	{
		// Create data to update section
		FProcMeshSectionUpdateData* SectionData = new FProcMeshSectionUpdateData;
		SectionData->TargetSection = InMeshSectionIndex;
		SectionData->NewVertexBuffer = Section.ProcVertexBuffer;
...
void UJetProceduralMeshComponent::ClearMeshSection(int32 SectionIndex)
{
	if (SectionIndex < ProcMeshSections.Num())
	{
...
void UJetProceduralMeshComponent::ClearAllMeshSections()
{
...
void UJetProceduralMeshComponent::SetMeshSectionVisible(int32 SectionIndex, bool bNewVisibility)
{
	if (SectionIndex < ProcMeshSections.Num())
	{
...
bool UJetProceduralMeshComponent::IsMeshSectionVisible(int32 SectionIndex) const
{
	return (SectionIndex < ProcMeshSections.Num()) ? ProcMeshSections[SectionIndex].bSectionVisible : false;
}

int32 UJetProceduralMeshComponent::GetNumSections() const
{
	return ProcMeshSections.Num();
}

void UJetProceduralMeshComponent::AddCollisionConvexMesh(TArray<FVector> ConvexVerts)
{
...
void UJetProceduralMeshComponent::ClearCollisionConvexMeshes()
{
...
void UJetProceduralMeshComponent::SetCollisionConvexMeshes(const TArray< TArray<FVector> >& ConvexMeshes)
{
...
void UJetProceduralMeshComponent::UpdateLocalBounds()
{
	FBox LocalBox(ForceInit);

	for (const FJetProcMeshSection& Section : ProcMeshSections)
	{
...
FPrimitiveSceneProxy* UJetProceduralMeshComponent::CreateSceneProxy()
{
...
int32 UJetProceduralMeshComponent::GetNumMaterials() const
{
	return ProcMeshSections.Num();
}


FJetProcMeshSection* UJetProceduralMeshComponent::GetProcMeshSection(int32 SectionIndex)
{
	if (SectionIndex < ProcMeshSections.Num())
	{
...
void UJetProceduralMeshComponent::SetProcMeshSection(int32 SectionIndex, const FJetProcMeshSection& Section)
{
	// Ensure sections array is long enough
	if (SectionIndex >= ProcMeshSections.Num())
	{
...
FBoxSphereBounds UJetProceduralMeshComponent::CalcBounds(const FTransform& LocalToWorld) const
{
...
bool UJetProceduralMeshComponent::GetPhysicsTriMeshData(struct FTriMeshCollisionData* CollisionData, bool InUseAllTriData)
{
	...
	{
		FJetProcMeshSection& Section = ProcMeshSections[SectionIdx];
...
bool UJetProceduralMeshComponent::ContainsPhysicsTriMeshData(bool InUseAllTriData) const
{
	for (const FJetProcMeshSection& Section : ProcMeshSections)
	{
...
UBodySetup* UJetProceduralMeshComponent::CreateBodySetupHelper()
{
...
void UJetProceduralMeshComponent::CreateProcMeshBodySetup()
{
...
void UJetProceduralMeshComponent::UpdateCollision()
{
	...
	if (bUseAsyncCook)
	{
		UseBodySetup->CreatePhysicsMeshesAsync(FOnAsyncPhysicsCookFinished::CreateUObject(this, &UJetProceduralMeshComponent::FinishPhysicsAsyncCook, UseBodySetup));
	}
	else
	{
...
void UJetProceduralMeshComponent::FinishPhysicsAsyncCook(bool bSuccess, UBodySetup* FinishedBodySetup)
{
...
UMaterialInterface* UJetProceduralMeshComponent::GetMaterialFromCollisionFaceIndex(int32 FaceIndex, int32& SectionIndex) const
{
	...
	if (FaceIndex >= 0)
	{
		...
		for (int32 SectionIdx = 0; SectionIdx < ProcMeshSections.Num(); SectionIdx++)
		{
			const FJetProcMeshSection& Section = ProcMeshSections[SectionIdx];
